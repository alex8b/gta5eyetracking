...
namespace Gta5EyeTracking.Features
{
	public class ExtendedView: IDisposable
	{
        ...
        private const float DecayRate = 0.7f;
		...
		public void ProcessFirstPersonVehicle()
		{
			...
			if (_settings.ExtendedViewEnabled)
			{
				...
				{
                    _extendedViewCamera.AttachTo(Game.Player.Character, (int)Bone.SKEL_ROOT, new Vector3(0, 0, 0.6f));
                    _forwardCamera.AttachTo(Game.Player.Character, (int)Bone.SKEL_ROOT, new Vector3(0, 0, 0.6f));
                }
				else
				{
					_extendedViewCamera.AttachTo(Game.Player.Character, (int)Bone.SKEL_ROOT, new Vector3(0, 0, 0.6f));
		...
		private void ApplyCameraRotation()
		{
            ...
            var rot = Geometry.OffsetRotation(GameplayCameraRotationFiltered,
                (-_gazePitch * _scaleGazePitch + _headPitchFiltered * HeadRotationScalar) * _decayScale,
                (-_gazeYaw * _scaleGazeYaw - _headYawFiltered * HeadRotationScalar) * _decayScale);
